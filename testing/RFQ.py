...
class RFQ():
	...
	def __init__(self, RFQId = None, CommodityId = None, CreatorId = None, SystemStatusId = None, EnquiryPersonId = None, DocumentId = None, Number = None, Name = None, Description = None, CreateDate = None, ClosingDate = None, ExpectedSupplyDate = None, PublishedDate = None, data_access = None):
		self.meta = MetaData()
		...
		self.content = Table(
			'RFQ', self.meta,
			Column('RFQId', Integer, primary_key = True),
	...
	def DBFetch(self, RFQId):
		try:
			if not self.IsInserted and not self.IsFetched:
				self._db_fetch_check()
				self._db_fetch(RFQId)
	...
	def Delete(self):
		try:
			if self.IsInserted or self.IsFetched:
				...
				raise DeleteError('The RFQ is neither fetched nor inserted.')
		except DeleteError as delete_error:
			raise delete_error

	def _db_insert_check(self):
		if self.IsDeleted:
			raise InsertError('The RFQ is deleted.')
	...
	def _db_insert(self):
		if not self.IsInserted and not self.IsFetched and not self.IsUpdated:
			...
			self.RFQId = result.inserted_primary_key
			self.IsInserted = True
			self.IsFetched = True

	def _db_fetch_check(self):
		if self.IsDeleted:
			raise FetchError('The RFQ is deleted.')

	def _db_fetch(self, RFQId):
		if not self.IsFetched:
			s = self.content.select().where(self.content.c.RFQId == self.RFQId)
			result = self.data_access.connection.execute(s)
			row = result.first()

			if row == None:
				raise FetchError('The RFQ does not exist. RFQ Id is {0}.'.format(str(RFQId)))
			else:
	...
	def _db_delete_check(self):
		if self.IsDeleted:
			raise DeleteError('The RFQ is deleted.')

	def _db_delete(self):
		s = self.content.delete().where(self.content.c.RFQId == self.RFQId)
	...
	def _db_update_check(self):
		if self.IsDeleted:
			raise UpdateError('The RFQ is deleted.')

	def _db_update(self):
		s = self.content.update().where(self.content.c.RFQId == self.RFQId).values(CommodityId = self.CommodityId, CreatorId = self.CreatorId, SystemStatusId = self.SystemStatusId, EnquiryPersonId = self.EnquiryPersonId, DocumentId = self.DocumentId, Number = self.Number, Name = self.Name, Description = self.Description, CreateDate = self.CreateDate, ClosingDate = self.ClosingDate, ExpectedSupplyDate = self.ExpectedSupplyDate, PublishedDate = self.PublishedDate)
		self.data_access.connection.execute(s)
